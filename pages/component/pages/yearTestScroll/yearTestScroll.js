      ...
      app.encryption({//交卷动作
        ...
        success: function (res) {
          ...
          let nums3 = 'topmenu[2].number'
           //合并数组
          let alltestID = [];
          alltestID = alltestID.concat(res.list.single_problem,res.list.multiple_problem,res.list.judge_problem,res.list.indefinite_problem,res.list.fill_problem,res.list.short_problem,res.list.scenes_problem);
          alltestID = alltestID.map((item)=>  item.problem_id )
            that.setData({
              ...
              indefinite_problem: res.list.indefinite_problem.length == 0 ? [] : res.list.indefinite_problem,//不定项
