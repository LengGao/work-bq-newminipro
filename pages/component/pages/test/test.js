...
endDrag(event) {
  ...
  bindsenceNext(){
    console.log(this.data.randerTitle); // 点击下一题增加+1
    if(this.data.randerTitle.child[ this.data.senceIndex ].problem_child_type == 2){//多选题在此提交答案
      if(this.data.randerTitle.hasSubmit){ // 表明已提交过答案
      }else{
        let multiselect = this.data.multiselect
        this.submitAnswer(multiselect,curID)
      }
    }
    ...
    console.log(this.data.randerTitle,index)
  ...
  bindsenceLast(){
    console.log(this.data.randerTitle); // 点击下一题减少-1
    if(this.data.senceIndex <= 0){
      this.setData({
        senceIndex: 1
      })
    }else{
